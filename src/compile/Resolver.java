...
public class Resolver {
	...
	private NodeType[] nodesType; // the type of every node, useful ,useless or
									// TBD
	private File file;
	...
	public void DeleteRedundantInformation() {
		...
		for (int i = 0; i < this.nodesNum; i++) {
			...
			if (position != -1) {
				...
				position += 6;
				String name = this.nodes.getNode(i).substring(position,
						position + length);
				...
				while (number != -1) {
					if (number != -1) {
						...
						if (this.nodesType[i] == NodeType.USEFUL
								&& this.nodesType[child - 1] == NodeType.TBD) {
							this.nodesType[child - 1] = NodeType.USEFUL;
						} else if (this.nodesType[i] == NodeType.USELESS
								&& this.nodesType[child - 1] == NodeType.TBD)
	...
	public void StandardizeASTText() {
		...
		List<Field> field;
		for (int i = 0; i < treeNodes.size(); i++) {
			// remove all identifier node
			if (treeNodes.get(i).getType().equals("identifier_node")) {
				treeNodes.remove(i--);
				continue;
				}
			...
			if (treeNodes.get(i).getType().equals("scope_stmt")) {
				...
				for (int j = field.size() - 1; j >= 0; j--)
					if (field.get(j).getName() != "begin"
							&& field.get(j).getName() != "end"
							&& field.get(j).getName() != "line"
							&& field.get(j).getName() != "next")
						field.remove(j);
			}
			// remove fields that point node is invalid
			for (int j = treeNodes.get(i).getField().size() - 1; j >= 0; j--) {
				if (!exist(treeNodes.get(i).getField().get(j).getValue(),
						numbers))
		...
		// if field is empty then remove the node
		for(int i = 0; i < treeNodes.size(); i++) {
			if(treeNodes.get(i).getField().isEmpty())
				treeNodes.remove(i--);
		}
		// change every '@number' to 'number'
		for (int i = 0; i < treeNodes.size(); i++)
			for (int j = 0; j < treeNodes.get(i).getField().size(); j++)
				treeNodes
						...
						.get(j)
						.setValue(
								treeNodes.get(i).getField().get(j).getValue()
										.substring(1));
		// number mapping
		for (int i = 0; i < treeNodes.size(); i++) {
			...
			for (int j = 0; j < treeNodes.size(); j++) {
				for (int k = 0; k < treeNodes.get(j).getField().size(); k++) {
					if (n == Integer.parseInt(treeNodes.get(j).getField()
							.get(k).getValue()))
						treeNodes.get(j).getField().get(k)
		...
		for(int i = 0; i < treeNodes.size(); i++) {
			for(int j = treeNodes.get(i).getField().size() - 1; j >= 0; j--) {
				if(Integer.parseInt(treeNodes.get(i).getField().get(j).getValue()) >= treeNodes.size())
					treeNodes.get(i).getField().remove(j);
			}
		}
		// for(int i = 0;i<treeNodes.size();i++)
	...
	public void EstablishAST() {
		...
		for (int i = 0; i < treeNodes.size(); i++) {
			for (int j = 0; j < treeNodes.get(i).getField().size(); j++) {
				int n = Integer.parseInt(treeNodes.get(i).getField().get(j)
						.getValue());
